const API_KEY = 'a08c5188a1fbdec2e9216570aa2679cf'; // <-- get a free key at openweathermap.org
...
const tempFullEl = document.getElementById('tempFull');
...
function formatTime(tsSeconds, tzOffsetSeconds) {
  ...
  const local = new Date(utc.getTime() + tzOffsetSeconds * 1000 - utc.getTimezoneOffset() * 60000);
...
function cityLocalTime(tzOffsetSeconds) {
  ...
  const local = new Date(utc.getTime() + tzOffsetSeconds * 1000);
  return local.toLocaleTimeString([], {
    hour: '2-digit',
    minute: '2-digit',
    second: '2-digit',
...
function renderWeather(data) {
  ...
  tempEl.textContent = main ? Math.round(main.temp) : '—'; // big temp
  tempFullEl.textContent = main ? main.temp.toFixed(2) : '—'; // exact temp
  feelsLikeEl.textContent = main ? main.feels_like.toFixed(2) : '—';
  ...
  timezoneEl.textContent = timezone ? `UTC +${timezone / 3600}` : '—';
